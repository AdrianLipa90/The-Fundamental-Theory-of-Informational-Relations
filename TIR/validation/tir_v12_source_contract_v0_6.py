#!/usr/bin/env python3
"""Fail-closed source contract for TIR v12.3 semantic frontier reconciliation."""
...
SYNC_MANIFEST = MONOGRAPH / "v12/MONOGRAPH_SYNC_V12_3.yaml"
SEMANTIC_REGISTRY = MONOGRAPH / "v12/SEMANTIC_FRONTIER_V12_3.json"
PASS5_AUDIT = MONOGRAPH / "v12/GREMLIN_PASS5_SEMANTIC_FRONTIER_AUDIT_20260918.md"
HISTORICAL_SYNC_MANIFEST = MONOGRAPH / "v12/MONOGRAPH_SYNC_V12_1.yaml"
PASS4_AUDIT = MONOGRAPH / "v12/GREMLIN_PASS4_BRANCH_MONOGRAPH_AUDIT_20260918.md"
...
def main() -> int:
    ...
    checks["master_is_v12_3_semantic_revision"] = "Version 12.3 Semantic Frontier Reconciliation" in master
    ...
    ch11 = read(CHAPTER_DIR / "ch11_action_architecture_coefficient_forcing.tex")
    checks["coefficient_parent_evaluation_source_bound"] = all(token in ch11 for token in (
        "TIR_COEFFICIENT_MAGNITUDE_PARENT_EVALUATION_V0_1.md",
        "tir_coefficient_magnitude_parent_evaluation_v0_1.py",
        "M(P_e)=(1,3,1,1)",
        "M(P_{e\\mu})=(0,5,2,8)",
        "M(P_{\\mu\\tau})=(0,3,1,7)",
    ))
    checks["coefficient_selector_remains_open"] = (
        "COEFFICIENT\\_TRANSITION\\_PARENT\\_SELECTOR" in ch11
        and r"\vTwelveStatus{B}{--}{OPEN}" in ch11
    )
    checks["coefficient_target_leakage_firewall_present"] = all(token in ch11 for token in (
        "observed particle masses",
        "observed Yukawa couplings",
        "already recovered coefficient tuple",
        "residual-to-target information",
    ))

    ch12 = read(CHAPTER_DIR / "ch12_charged_leptons_neutrinos.tex")
    checks["charged_lepton_parent_evaluation_not_confused_with_physical_pass"] = all(token in ch12 for token in (
        "parent-count arithmetic",
        "transition-parent selector",
        "precision verdict below",
        r"\vTwelveStatus{C}{RETROSPECTIVE}{FAIL}",
    ))
    ...
    checks["completion_frontier_v12_3_semantic_current"] = all(token in ch21 for token in (
        "v12.3 semantic reconciliation",
        "RF-E13",
        "RF-E24",
        "magnitude evaluation is",
        "transition-parent selector remains OPEN",
        "Hypercharge relative uniqueness",
        "neutrino action repair",
        "physical metric/source coupling",
        "native Li--Weil",
    ))
    checks["stale_einstein_open_removed"] = "The former Einstein constraint/evolution item is stale" in ch21
    checks["neutrino_repair_closed_physical_open"] = (
        "derivational repair is CLOSED" in ch21
        and "absolute-neutrino mass scale remains OPEN" in ch21
    )
    ...
    appendix_f = APPENDIX_DIR / "appF_gremlin_pass4_repository_consolidation.tex"
    appendix_f_text = read(appendix_f) if appendix_f.is_file() else ""
    checks["appendix_f_substantive"] = appendix_f.is_file() and len(appendix_f_text) >= 3000 and "GREMLIN Pass-4 Repository Consolidation" in appendix_f_text
    checks["appendix_f_branch_sweep_bound"] = all(token in appendix_f_text for token in ("81=77_{\\rm contained}+4_{\\rm unique}", "jarlskog-closure-v01", "research/xf9-preprint-spider-bib-20260918"))
    checks["appendix_f_gremlin_nonpromotion"] = "authority=CANDIDATE\\_ONLY" in appendix_f_text and "canon\\_allowed=false" in appendix_f_text
    ...
    checks["sync_manifest_bound_to_v12_3_semantics"] = (
        'version: "12.3-semantic-frontier-reconciliation"' in sync_manifest
        and 'TIR: "45b54c0c8aa60ec92607e6132b69aca9ab766d75"' in sync_manifest
        and 'RFC: "ce4af9ddd480ea40dfb5a3ee26ed396a58cb0e54"' in sync_manifest
        and 'SOH: "d99545aa447ef86bc253d8241a3fee9adb8a42c1"' in sync_manifest
        and 'merge_to_main: false' in sync_manifest
    )
    semantic_registry = json.loads(read(SEMANTIC_REGISTRY)) if SEMANTIC_REGISTRY.is_file() else {}
    checks["semantic_registry_v12_3"] = (
        semantic_registry.get("schema") == "TIR_MONOGRAPH_SEMANTIC_FRONTIER_V12_3"
        and len(semantic_registry.get("gates", [])) == 14
        and semantic_registry.get("pinned_heads", {}).get("RFC") == "ce4af9ddd480ea40dfb5a3ee26ed396a58cb0e54"
    )
    pass5_audit = read(PASS5_AUDIT) if PASS5_AUDIT.is_file() else ""
    checks["pass5_audit_root_cause_and_firewall"] = (
        "Root cause" in pass5_audit
        and "software PASS does not imply a physical PASS" in pass5_audit
        and "open physical/source realization must not be used to relabel an already completed mathematical derivation as OPEN" in pass5_audit
    )
    historical_sync = read(HISTORICAL_SYNC_MANIFEST) if HISTORICAL_SYNC_MANIFEST.is_file() else ""
    checks["historical_v12_1_sync_preserved"] = (
        'version: "12.1-repository-sync"' in historical_sync
        and 'baseline_main: "0f63c823961a58770de22d14b5f53e2bfa38a9b3"' in historical_sync
    )
    ...
    sm_v02 = ROOT / "TIR/standard_model/TIR_SM_RECONCILIATION_LEDGER_V0_2.md"
    sm_text = read(sm_v02) if sm_v02.is_file() else ""
    checks["sm_ledger_v0_2_narrows_coefficient_gate"] = sm_v02.is_file() and all(token in sm_text for token in (
    ...
    receipt = {
        "schema": "TIR_V12_SOURCE_CONTRACT_V0_6",
        "technical_status": status,
        "supersedes": "TIR_V12_SOURCE_CONTRACT_V0_5",
        "master": str(MASTER.relative_to(ROOT)),
        "chapter_count": len(CHAPTERS),
        "appendix_e": str(appendix_e.relative_to(ROOT)),
        "appendix_f": str(appendix_f.relative_to(ROOT)),
        "semantic_registry": str(SEMANTIC_REGISTRY.relative_to(ROOT)),
        "pass5_audit": str(PASS5_AUDIT.relative_to(ROOT)),
        "pass4_audit": str(PASS4_AUDIT.relative_to(ROOT)),
        "physical_promotion": False,
        "coefficient_transition_parent_selector_closed": False,
