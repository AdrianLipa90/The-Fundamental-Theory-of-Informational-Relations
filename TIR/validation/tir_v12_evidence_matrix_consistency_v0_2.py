...
ROWS = {
    ...
    "hypercharge_source_map": ("B", "--", "OPEN"),
    "standard_local_anomaly_sums_witten": ("A", "--", "PASS"),
    "legacy_a5_baryon_current": ("D", "RETROSPECTIVE", "QUARANTINED"),
...
def main() -> None:
    ...
    checks["row_count_is_32"] = len(ROWS) == 32
    ...
    checks["hypercharge_source_remains_open"] = ROWS["hypercharge_source_map"] == ("B", "--", "OPEN")
    checks["standard_anomaly_algebra_pass"] = ROWS["standard_local_anomaly_sums_witten"] == ("A", "--", "PASS")
    checks["legacy_a5_quarantined"] = ROWS["legacy_a5_baryon_current"] == ("D", "RETROSPECTIVE", "QUARANTINED")
    checks["legacy_map_targets_valid"] = set(LEGACY_MAP.values()) <= VERDICTS

    status = "PASS" if all(checks.values()) else "FAIL"
    receipt = {
        "schema": "TIR_V12_EVIDENCE_MATRIX_CONSISTENCY_V0_2",
        "status": status,
        "current_evidence_owner": "TIR/monograph/v12/chapters/ch19_unified_evidence_matrix.tex",
        "supersedes": "TIR_V12_EVIDENCE_MATRIX_CONSISTENCY_V0_1",
        "row_count": len(ROWS),
