...
## 14. Differential frame/curvature compatibility
...
X_{36}
\to
\big(X_{\rm frame},Q_{\rm alg}\big)
\]

is not itself a gravitational field equation.

The new gate `TIR_DIFFERENTIAL_FRAME_CURVATURE_COMPATIBILITY_V0_1` defines

\[
S_{\rm alg}=JQ_{\rm alg}
\]

and requires comparison with curvature independently generated by the admitted coframe/Levi-Civita chain:
...
Physical compatibility requires \(\mathcal C=0\) in the declared basis/signature/normalization.

Two exact controls are recorded:

1. the constant canonical tetrahedral relation matrix has
   \(\|Q\|_F^2=10/3\), while a constant local coframe has
   \(R_{\rm LC}=0\); therefore it **fails** differential compatibility as expected;

2. the curved local control
   \[
   ...
   \right)
   \]
   for \(S^2\times\mathbb R^2\) gives
   \(R_{\hat0\hat1\hat0\hat1}=1\) at the chart origin and is exactly recovered by the 20D algebraic-curvature channel.

Thus the narrowed source gate is now:

\[
\boxed{
\text{source-derived variable relation field }X(x)
\to
(H(x),Q(x))
\to
e(x)
...
A static pointwise 36D pattern is insufficient. Spatial/temporal variation or separately admitted nontrivial connection/gluing data are required.
