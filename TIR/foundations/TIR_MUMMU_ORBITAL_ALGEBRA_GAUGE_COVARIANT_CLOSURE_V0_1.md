...
The remaining mathematical gate is no longer generic “choose a common frame”.
It is narrower:

\[
\boxed{
\text{derive the admitted }\rho:
\mathsf{Path}(\mathcal O)\to SU(2)
\text{ uniquely from }(\Pi,\phi,\theta,\tau)
}
\]

and determine which Lagrange-hypernode admission rule fixes the allowed loop set
\(\mathcal C_{\mathcal L}\) without fitted coefficients.
