...
## 16. Adaptive coupling variational closure

The form of the source adaptive-coupling law is now given an exact local
variational representation in

`TIR_MUMMU_ADAPTIVE_COUPLING_VARIATIONAL_CLOSURE_V0_1.md`.

At a frozen source section the Hebbian+decay substep is
...
mathcal F_{HD}
=
-lambda_Hlangle g,C_Phiangle_F
+
rac{gamma}{2}|g-g_0|_F^2.
]

The weak-channel boost is a second gradient substep generated by a linear
potential, and both are followed by the exact box projection implemented by
entrywise clipping.

Thus the source mechanism

[
	ext{phase-conditioned relation}
	o
dot g
	o
dot J
...
now has an exact local variational form.

The numerical gain values remain source policy parameters. The remaining
