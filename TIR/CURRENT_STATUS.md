...
### 2026-09-24 response and dimensional closure firewalls
...
RF-F22 gives the exact positive-projector acceleration threshold

\[
\frac{R_0+3R_s}{\mu_\vartheta^2}+(S_0+3S_s)<-\frac12.
\]

AB and fixed-map Berry channels admit exact zero metric-response branches; RF-F19 admits S_vartheta=0 on its independent-variation branch. No current theorem derives a nonzero Euler/spin metric functional. If the same ADM lapse is reused inside the projector calibration during the same metric variation, RF-F18 self-normalization is recovered and the projector stress vanishes exactly. Separately, C_Lambda has dimension T^-4, so dimensionless winding/Chern/holonomy data can only determine C_Lambda/Omega_*^4 after an independent frequency scale Omega_* is supplied.
