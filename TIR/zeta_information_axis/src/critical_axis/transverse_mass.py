...
These implications are ordinary calculus statements.  The current external
claim that the Riemann Xi kernel is globally strictly log-concave is tracked
separately at the research-firewall level.  Runtime derivatives below use the
finite `riemann_phi` evaluator and therefore provide numerical diagnostics.
"""
...
def gaussian_mass_envelope(
    ...
    *,
    ...
    The value is a rigorous upper envelope only when the caller has
    independently established -d^2 log(Phi)/du^2 >= lambda throughout the
    full interval [a-|b|, a+|b|].  The function deliberately does not infer
    or certify that premise.
    """
    aa, bb = _validate_diagonal_coordinates(a, b)
