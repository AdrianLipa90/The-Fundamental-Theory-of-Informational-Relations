...
The old conditional two-state theorem treated a \(\pi\) relative zero phase as an assumption. XF-1 already replaced the representation debt by a canonical two-branch Xi-kernel identity. XF-8 further shows that an operator-derived Xi representation carries the boundary antiphase
