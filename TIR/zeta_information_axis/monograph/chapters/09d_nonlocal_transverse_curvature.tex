...
The solver records the chain
\begin{center}
\texttt{Q\_Xi strict positivity}
$\Longleftrightarrow$
\texttt{strict transverse convexity}
$\Longrightarrow$
\texttt{vertical growth}
$\Longrightarrow$
\texttt{RH},
\end{center}
while retaining the first globally quantified condition as \status{OPEN}.  Thus XF-5 changes the representation of the proof obligation and preserves its evidential status.
...
Route & Status\\
\midrule
Global correlated sign control & \status{ACTIVE}\\
Exact resummation preserving sector correlation & \status{ACTIVE}\\
Positive representation of the fully coupled integral & \status{ACTIVE}\\
Universal independent phase-aligned block positivity & \status{LITERATURE\_NO\_GO}\\
Finite block sampling & \status{NUMERICAL\_DIAGNOSTIC}\\
\bottomrule
\end{tabular}
\end{center}
...
The module \texttt{critical\_axis.nonlocal\_curvature} exposes
\begin{itemize}
\item \texttt{xi\_transverse\_curvature(x,y)}, implementing Eq.~\eqref{eq:xf5-curvature-identity};
\item \texttt{xi\_transverse\_curvature\_direct(x,y)}, independently differentiating $|\Xi|^2$ for regression;
\item \texttt{theta\_growth\_kernel(x,y,a,b)};
\item \texttt{theta\_curvature\_kernel(x,y,a,b)}, implementing Eq.~\eqref{eq:xf5-curvature-kernel};
\item \texttt{theta\_curvature\_integrand}, evaluating one local contribution with fail-closed diagonal coordinates.
\end{itemize}
