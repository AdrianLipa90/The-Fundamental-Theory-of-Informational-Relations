...
proton mass $E_{\text{proton}} = 938.272$~MeV.  Additional external anchors and conversion inputs are declared explicitly in the\npublication-candidate monograph; results depending on them are not independent\npredictions of those anchors.
...
(Sec.~\ref{sec:baryons}), eliminating those particular continuous baryon scales while leaving the model's discrete structural complexity explicit.
