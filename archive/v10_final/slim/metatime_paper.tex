...
0.48\,\%), baryon octet (1.72\,\%), baryon decuplet (0.34\,\%), pseudoscalar and
vector mesons (0.58\,\%), neutrino PMNS parameters (all within $1.6\sigma$),
CKM matrix magnitudes (mean $\sigma = 0.78$; the CP phase $\delta = 73.6^\circ$
differs from the PDG $65.6^\circ$ at $\sigma = 5.35$ and remains unresolved),
electroweak gauge bosons and the Higgs
($M_H = 125.94$~GeV, $+0.55\,\%$; $M_W$ and $M_Z$ show a systematic $-4\%$
deviation; the fine-structure constant is now derived as
...
bound---this variant is in tension with experiment),
...
\beta &= -\alpha\,\frac{L_3 - 1}{L_3 - 1 + L_4/L_5}, \label{eq:beta} \\
\gamma &= E_{\text{proton}}\,\kappa\,(L_3 - 1 + L_4/L_5). \label{eq:gamma}
\end{align}
These give
\begin{align}
\alpha &= 938.272 \times 0.00919315 \times (49 - 9 - 25 + 7)
       = 189.77\ \mev, \\
\beta &= -189.77 \times \frac{6}{6.4}
       = -177.91\ \mev, \\
\gamma &= 938.272 \times 0.00919315 \times 6.4
       = 55.20\ \mev.
\end{align}
...
p               & 965.41   & 938.27   & +2.89\% \\
n               & 965.41   & 939.57   & +2.75\% \\
\Lambda         & 1115.72  & 1115.68  & 0.00\% \\
\Sigma^+        & 1226.12  & 1189.37  & +3.09\% \\
\Sigma^0        & 1226.12  & 1192.64  & +2.81\% \\
\Sigma^-        & 1226.12  & 1197.45  & +2.39\% \\
\Xi^0           & 1321.23  & 1314.86  & +0.48\% \\
\Xi^-           & 1321.23  & 1321.71  & -0.04\%
\end{array}
\]
Mean absolute error: $1.72\%$.

The $\Lambda$ mass agrees to within $0.04\ \mev$ ($0.00\%$), confirming
that the $M_0$ derivation \eqref{eq:M0} is consistent.  The $p/n$ and
$\Sigma$ states show a systematic $+2.8\%$ offset, indicating that the
GMO coefficients $\beta$ and $\gamma$ from the J-KJ axial eigenvalues
overestimate the hypercharge and isospin splittings.  Refining the
connection between the axial eigenvalues and the L-constant expressions
for $\beta$ and $\gamma$ is left for future work.
...
$\sigma = 5.35$).  \textbf{This remains unresolved:} while the magnitudes
fit well ($\sigma = 0.78$), the CP phase shows significant tension
and requires further refinement of the $V_{cb}$ structural formula
or a revision of the phase-generation mechanism.
...
The CP phase $\delta = 73.6^\circ$ ($\sigma = 5.35$) is excluded
from this mean and is a known open issue.
...
The gauge couplings are $e = \sqrt{4\pi\alpha}$ with
$\alpha$ from \eqref{eq:alpha_metatime}, $g = e/\sin\theta_W$,
$g' = e/\cos\theta_W$, giving
\begin{align}
...
Poincar\'{e} disk connection is in progress.  The fine-structure
constant is now derived (\eqref{eq:alpha_metatime}), but the
$SU(2)$ coupling remains structurally determined.
...
$L_4/L_3$ raised to the heaviest active flavor.  The choice of exponent
$b = 13$ (the bottom-quark prime) is structurally motivated but
alternatives exist (charm $c = 11$ gives $\theta_c = 9.5 \times 10^{-7}$,
which is excluded; top $t = 17$ gives $\theta_t = 6.4 \times 10^{-12}$,
which would be even smaller).  The present choice represents the
heaviest quark that hadronizes.
...
(90\% CL).  \textbf{This constitutes a tension with existing data.}
Next-generation nEDM experiments with sensitivity to
$10^{-28}\ e\cdot\text{cm}$ will definitively test this prediction,
but in its present form the framework is in tension with the observed
bound.  No axion is required: $\theta$ is naturally small from flavor
geometry, though the specific exponent requires further derivation.
...
\item Baryon octet ($1.72\%$ mean error, dominated by $+2.8\%$
systematic offset in $p/n$ and $\Sigma$ from the GMO coefficients;
$M_0$ now derived, no free parameter) and decuplet ($0.34\%$ mean
error, $M_0'$ also derived);
\item CKM matrix magnitudes with mean $\sigma = 0.78$; the CP phase
$\delta = 73.6^\circ$ ($\sigma = 5.35$) remains unresolved;
...
the current experimental bound---this is a \textbf{tension} with
existing data, testable in next-generation nEDM experiments;
...
from the $SU(2)$ coupling $g$, whose full geometric derivation
from the Poincar\'{e} disk connection is in progress.  The
fine-structure constant is now derived (Eq.~\eqref{eq:alpha_metatime}),
but $g$ remains the bottleneck for electroweak precision.
...
eliminating the last external gauge input.  The $SU(2)$ coupling
$g$ that enters $M_W/M_Z$ remains structurally determined.
\item The strong CP prediction $d_n$ is a factor $\sim 10$ above the
current nEDM bound, placing this variant in tension with experiment.
...
continuous free parameters, but the $+2.8\%$ systematic offset in
$p/n$ and $\Sigma$ masses indicates that the GMO coefficients
$\beta$ and $\gamma$ from the J-KJ axial eigenvalues require
further refinement.
