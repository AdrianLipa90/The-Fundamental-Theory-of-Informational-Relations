# Foundations Bibliography

## Status
`defined`

## Purpose
This directory contains canonical bibliography surfaces for the Foundations repository.
Bibliography records support explanation, attribution, and literature mapping. They do **not** by themselves upgrade any repository object to `derived` or `verified` status.

## Canonical files
- `bibliography/README.md`
- `bibliography/foundations_seed.bib`

## Current seed coverage
### Projective / geometric quantum foundations
- Kibble
- Berry
- Aharonov–Anandan
- Wilczek–Zee

### Rotating flow / superfluid context
- Pethick & Smith
- rotating-condensate chapter
- Coriolis reference

### Tau / kernel / spectral bridge context
- Penrose
- Chung
- Fiedler

### Object-state extraction / information / networks
- Shannon
- Newman
- Brin & Page

## Use rule
1. Bibliography entries may justify terminology, analogies, or standard background.
2. Bibliography entries do not override canonical epistemic status in axioms/definitions/derivations.
3. Any new external source used repeatedly in the formal layers should be registered here.
